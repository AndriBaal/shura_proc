...
fn position_field(data_struct: &syn::DataStruct, attr_name: &str) -> Option<(String, String)> {
...
impl ComponentDerive for {struct_name} {{
    fn inner(&self) -> &dyn shura::BaseComponent {{
        &self.{field_name}
    }}

    fn inner_mut(&mut self) -> &mut dyn shura::BaseComponent {{
        &mut self.{field_name}
    }}
}}
